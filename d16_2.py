...
def run(lines: list[str], max_time: int = 26) -> int:
    ...
    flags = {v: 1 << flag for flag, v in enumerate(valve_names)}

    to_check = {0: State("AA", max_time, 0)}
    ...
    while to_check:
        ...
        for v in valve_names:
            ...
            if new_visited in to_check:
                ...
                to_check[new_visited] = new_state
            if new_visited in final_states:
                final_states[new_visited] = max(final_states[new_visited], new_state.sum_flow)
            else:
                final_states[new_visited] = new_state.sum_flow
    print(f"paths done, found paths: {len(final_states)}")
    res = [flow1 + flow2 for visited1, flow1 in final_states.items() for visited2, flow2 in final_states.items()
