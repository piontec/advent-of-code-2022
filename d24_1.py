...
def run(lines: list[str]) -> int:
    ...
    def next_edges(vertex: Pos, t: int) -> Iterator[State]:
        new_t = t + 1
    ...
    start_state = State(start_point, 0)
    states: set[State] = {start_state}
    best_time = math.inf
    checked_states: set[State] = {start_state}
    while len(states) > 0:
        # choose best state to evaluate
        best_state = min(states, key=lambda s: heuristic(s.pos))
        states.remove(best_state)
        checked_states.add(best_state)
        # expand it
        next_states = set(next_edges(best_state.pos, best_state.t))
        found = set(filter(lambda s: s.pos == exit_point, next_states))
        if found:
